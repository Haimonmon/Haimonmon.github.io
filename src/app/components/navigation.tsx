import { useState, type JSX, Activity } from "react";
    ...
    return (
        <div className="h-16.5 w-full fixed top-0">
            ...
            <div className="h-16.5 w-full top-0 flex items-center justify-between pl-2 pr-2 fixed bg-dark-coded z-1">
                ...
                tablet:hidden small-desktop:hidden" 
                src="public/images/icons/burger-menu.png" 
                alt="closed burger icon" 
                onClick={() => setMenuClose(!isMenuClosed)}/>
            </div>
            
            <div className={`text-[2.5rem] font-[Fira_Code] text-right pr-7 pl=2 flex flex-col h-full w-full bg-dark-coded z-1 fixed top-16.5 
                ${isMenuClosed 
                ? '[clip-path:inset(0_0_100%_0)]' 
                : '[clip-path:inset(0_0_0_0)]'} 
                motion-safe:transition-[clip-path] duration-800 ease-in-out 
                tablet:hidden small-desktop:hidden large-desktop:hidden`}>
